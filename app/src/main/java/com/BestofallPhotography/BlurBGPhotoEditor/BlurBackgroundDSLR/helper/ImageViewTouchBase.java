...
public class ImageViewTouchBase extends ImageView implements IDisposable {
    ...
    private float INITIAL_ZOOM = 1f;
    protected float X_CENTER = 0.5f;
    protected float Y_CENTER = 0.5f;
    ...
    public void setImageRotateBitmapReset(final RotateBitmap bitmap, final boolean reset, final Matrix initial_matrix) {
        Log.d(LOG_TAG, "setImageRotateBitmapReset");
        if (getWidth() <= 0) {
            this.mOnLayoutRunnable = new Runnable() {
                public void run() {
                    ImageViewTouchBase.this.setImageBitmapReset(bitmap.getBitmap(), bitmap.getRotation(), reset, initial_matrix);
                }
            };
            return;
        }
    ...
    protected void moveTo(float centerX, float centerY) {
        postScale(0.0f, centerX, centerY);
        onZoom(getScale());
        center(true, true);
    }
    ...
    public boolean zoomTo(float scale, final float centerX, final float centerY, final float durationMs) {
        if (mBitmapDisplayed.getBitmap() == null) {
            return false;
        }
        ;
    ...
    public void scrollTo(final float centerX, final float centerY, final float durationMs) {
        // Log.d( LOG_TAG, "zoomTo: " + scale + ", " + centerX + ": " + centerY );
        final long startTime = System.currentTimeMillis();
        //final float incrementPerMs = ( scale - getScale() ) / durationMs;
        final float oldScale = getScale();
    ...
    public void setInitialScale(float INITIAL_ZOOM, float X_CENTER, float Y_CENTER) {
        this.INITIAL_ZOOM = INITIAL_ZOOM;
        this.Y_CENTER = X_CENTER;
